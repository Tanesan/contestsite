...
var $window = $(window), windowHeight = $window.height(), elements = [], $bully,
    lastScrollY = (window.pageYOffset || document.documentElement.scrollTop) -
                  (document.documentElement.clientTop || 0),
    current = 0, inversed = false, frameRendered = true;
...
$current =
    $('<div class="c-bully__bullet c-bully__bullet--active">').appendTo($bully);
...
(function update() {
  if (frameRendered !== true) {
    ...
    if (count !== current) {
      ...
      setTimeout(function() { $current.addClass("c-bully__bullet--squash"); });
...
function reloadAll() {
  $.each(elements, function(i, element) { element._reloadElement(); });
}
...
$window.on("load resize", function() { reloadAll(); });
...
Bully.prototype = {
  ...
  onClick : function() {
    var self = this, $target = $("html, body");
    ...
    $target.animate({scrollTop : this._calcTop(self.offset.top)},
